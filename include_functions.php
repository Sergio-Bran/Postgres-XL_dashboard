...
function getDatabaseSessions($strDB, $strDBName, $blnShowSQL = false){
  ...
  if(testPort($arrDBConns[$strDB]['DBIP'],$arrDBConns[$strDB]['DBPort'],.5)){
    ...
    $strSubSQL = '';
    if($arrDBConns[$strDB]['DBVersion'] == '10.1xl'){
      $strSubSQL = "SELECT b.datname
          ...
          JOIN pg_database as b on a.datid = b.oid WHERE b.datname = '".$strDBName."'";
    }
    if($strSubSQL == ''){
    ...
    if(!$objDBResults){
      return '<div class="alert alert-error">Error pulling stats from node! Had an issue pulling activity for <strong>' . $strDBName . '</strong></div>';
    }
...
function testPing($strServer){
  $strResult = exec('ping -c 1 '.$strServer, $outcome, $intStatus);
  if($intStatus == 0) {
    return true;
  }else{
    return false;
